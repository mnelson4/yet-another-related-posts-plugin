...
function narpp_load_after_other_plugins()
{
    ...
    if (defined('YARPP_VERSION')) {
        ...
        add_action(
            'admin_notices',
            function () {
                echo '<div class="notice notice-error">' . esc_html__(
                        'Not Another Related Posts Plugin cannot run while its predecessor, Yet Another Related Posts Plugin, is active. Please deactivate Yet Another Related Posts Plugin.',
                        'narpp'
                    ) . '</div>';
            }
        );
