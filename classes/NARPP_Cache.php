...
abstract class NARPP_Cache
{
    ...
    protected function sql($reference_ID = false, $args = array())
    {
        ...
        if (isset($weight['body']) && isset($weight['body']) && (int) $weight['body']) {
            $newsql .= " + (MATCH (post_content) AGAINST ('" . esc_sql($keywords['body']) . "')) * " . absint(
                    $weight['body']
                );
        }

        if (isset($weight['body']) && isset($weight['title']) && (int) $weight['title']) {
            $newsql .= " + (MATCH (post_title) AGAINST ('" . esc_sql($keywords['title']) . "')) * " . absint(
                    $weight['title']
                );
        }
        ...
        $newsql .= " where post_status in ( 'publish', 'static' ) and ID NOT IN ('" . implode(
                "','",
                $posts_to_exclude
            ) . "')";
    ...
    private function tax_criteria($reference_ID, $taxonomy)
    {
        ...
        return "count(distinct if( terms.term_taxonomy_id in (" . join(
                ',',
                $tt_ids
            ) . "), terms.term_taxonomy_id, null ))";
    }
    ...
    */
    function white($filter)
    {
        ...
        if (is_null($blacklist) || is_null($blackmethods)) {
            $yarpp_blacklist = array(
                'diggZEt_AddBut',
                'reddZEt_AddBut',
                'dzoneZEt_AddBut',
                'wp_syntax_before_filter',
                'wp_syntax_after_filter',
                'wp_codebox_before_filter',
                'wp_codebox_after_filter',
                'do_shortcode',
                'sharing_display',
                'really_simple_share_content',
            );//,'insert_tweet_this'
    ...
    /* FYI, apply_filters_if_white was used here to avoid a loop in apply_filters('the_content') > YARPP::the_content() > YARPP::related() > NARPP_Cache::body_keywords() > apply_filters('the_content').*/
