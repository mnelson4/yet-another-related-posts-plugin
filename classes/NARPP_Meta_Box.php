...
class NARPP_Meta_Box
{
    ...
    public function weight($option, $desc)
    {
        ...
        echo "<option {$fulltext} value='no'" . ((! $weight) ? ' selected="selected"' : '') . "  >" . __(
                "do not consider",
                'narpp'
            ) . "</option>";
        echo "<option {$fulltext} value='consider'" . (($weight == 1) ? ' selected="selected"' : '') . "  > " . __(
                "consider",
                'narpp'
            ) . "</option>";
        echo "<option {$fulltext} value='consider_extra'" . (($weight > 1) ? ' selected="selected"' : '') . "  > " . __(
                "consider with extra weight",
                'narpp'
            ) . "</option>";
        echo "</select></div></div>";
    }
    ...
    public function displayorder($option, $class = null)
    {
        ...
        ?>
        <option value="score DESC" <?php echo($order === 'score DESC' ? ' selected="selected"' : '') ?>><?php _e(
                "score (high relevance to low)",
                'narpp'
            ); ?></option>
        <option value="score ASC" <?php echo($order === 'score ASC' ? ' selected="selected"' : '') ?>><?php _e(
                "score (low relevance to high)",
                'narpp'
            ); ?></option>
        <option value="post_date DESC" <?php echo($order === 'post_date DESC' ? ' selected="selected"'
            : '') ?>><?php _e("date (new to old)", 'narpp'); ?></option>
        <option value="post_date ASC" <?php echo($order === 'post_date ASC' ? ' selected="selected"' : '') ?>><?php _e(
                "date (old to new)",
                'narpp'
            ); ?></option>
        <option value="post_title ASC" <?php echo($order === 'post_title ASC' ? ' selected="selected"'
            : '') ?>><?php _e("title (alphabetical)", 'narpp'); ?></option>
        <option value="post_title DESC" <?php echo($order === 'post_title DESC' ? ' selected="selected"'
            : '') ?>><?php _e("title (reverse alphabetical)", 'narpp'); ?></option>
        <?php
        echo "</select></div></div>";
    }
}