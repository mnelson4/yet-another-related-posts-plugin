...
class NARPP_Cache_Tables extends NARPP_Cache
{
    ...
    public function stats()
    {
        ...
        return wp_list_pluck(
            $wpdb->get_results(
                "select num, count(*) as ct from (select 0 + if(id = 0, 0, count(ID)) as num from {$wpdb->prefix}yarpp_related_cache group by reference_ID) as t group by num order by num asc",
                OBJECT_K
            ),
            'ct'
        );
    ...
    public function clear($reference_IDs)
    {
        ...
        $wpdb->query(
            "delete from {$wpdb->prefix}" . YARPP_TABLES_RELATED_TABLE . " where reference_ID in (" . implode(
                ',',
                $reference_IDs
            ) . ")"
        );
    ...
    protected function update($reference_ID)
    {
        ...
        $wpdb->query(
            "insert into {$wpdb->prefix}" . YARPP_TABLES_RELATED_TABLE . " (reference_ID,ID,score) " . $this->sql(
                $reference_ID
        ...
        if ($wpdb->rows_affected) {
            ...
            if ($this->core->debug) {
                echo "<!--NARPP just set the cache for post $reference_ID-->";
            }
            ...
            $wpdb->query(
                "insert into {$wpdb->prefix}" . YARPP_TABLES_RELATED_TABLE . " (reference_ID,ID,score) values ($reference_ID,0,0) on duplicate key update date = now()"
            );
    ...
    public function related($reference_ID = null, $related_ID = null)
    {
        ...
        if (! is_null($reference_ID) && ! is_null($related_ID)) {
            $results = $wpdb->get_col(
                "select ID from {$wpdb->prefix}" . YARPP_TABLES_RELATED_TABLE . " where reference_ID = $reference_ID and ID = $related_ID"
            );
        ...
        if (! is_null($reference_ID)) {
            return $wpdb->get_col(
                "select distinct ID from {$wpdb->prefix}" . YARPP_TABLES_RELATED_TABLE . " where reference_ID = $reference_ID and ID != 0"
            );
        }
        ...
        if (! is_null($related_ID)) {
            return $wpdb->get_col(
                "select distinct reference_ID from {$wpdb->prefix}" . YARPP_TABLES_RELATED_TABLE . " where ID = $related_ID"
            );
        }
