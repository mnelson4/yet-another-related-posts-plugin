...
class NARPP_Cache_Postmeta extends NARPP_Cache
{
    ...
    public function stats()
    {
        ...
        return wp_list_pluck(
            $wpdb->get_results(
                "select num, count(*) as ct from (select 0 + if(meta_value = '" . YARPP_NO_RELATED . "', 0, substring(substring_index(meta_value,':',2),3)) as num from `{$wpdb->postmeta}` where meta_key = '" . YARPP_POSTMETA_RELATED_KEY . "') as t group by num order by num asc",
                OBJECT_K
            ),
            'ct'
        );
    ...
    public function where_filter($arg)
    {
        ...
        $arg = preg_replace(
            "!{$wpdb->posts}.ID = \d+!",
            "{$wpdb->posts}.ID in (" . join(',', $this->related_IDs) . ")",
            $arg
        );
    ...
    public function related($reference_ID = null, $related_ID = null)
    {
        ...
        if (! is_null($related_ID)) {
            return $wpdb->get_col(
                "select post_id from `{$wpdb->postmeta}` where meta_key = '" . YARPP_POSTMETA_RELATED_KEY . "' and meta_value regexp 's:2:\"ID\";s:\d+:\"{$related_ID}\"'"
            );
        }
