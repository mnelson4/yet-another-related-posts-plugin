...
class NARPP_Core
{
    ...
    public function __construct()
    {
        ...
        $this->cache_bypass = new NARPP_Cache_Bypass($this);
         ...
         */
        if ($this->diagnostic_using_thumbnails() && (! ($dimensions = $this->thumbnail_dimensions(
                )) || isset($dimensions['_default']))) {
            $width = 120;
            $height = 120;
            $crop = true;
         ...
         */
        if (is_admin()) {
            require_once(YARPP_DIR . '/classes/NARPP_Admin.php');
            $this->admin = new NARPP_Admin($this);
            $this->enforce();
        }
    }
    ...
    public function enabled()
    {
        ...
        if (! (bool) $this->diagnostic_fulltext_disabled()) {
    ...
    public function diagnostic_fulltext_indices()
    {
        global $wpdb;
        $wpdb->get_results(
            "SHOW INDEX FROM {$wpdb->posts} WHERE Key_name = 'yarpp_title' OR Key_name = 'yarpp_content'"
        );
    ...
    public function upgrade_3_4_4b4()
    {
        ...
        $options['recent'] = ($options['recent_only']) ? $options['recent_number'] . ' ' . $options['recent_units']
            : false;
    ...
    public function the_excerpt_rss($content)
    {
        ...
        return $content . $this->clean_pre(
                $this->display_related(null, array('post_type' => $type, 'domain' => 'rss'), false)
            );
    }
