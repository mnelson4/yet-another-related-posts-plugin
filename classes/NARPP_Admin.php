...
class NARPP_Admin
{
    ...
    function __construct(&$core)
    {
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'flush' && check_ajax_referer(
                                                                         'yarpp_cache_flush',
                                                                         false,
                                                                         false
                                                                     ) !== false) {
        ...
        if (isset($_GET['action']) && $_GET['action'] === 'copy_templates' && check_ajax_referer(
    ...
    public function enqueue()
    {
        ...
        if (! is_null($screen) && $screen->id === 'settings_page_yarpp') {
            ...
            wp_enqueue_style(
                'narpp_options',
                plugins_url('style/options_basic.css', dirname(__FILE__)),
                array(),
                $version
            );
            ...
            wp_enqueue_script(
                'narpp_options',
                plugins_url('js/options_basic.js', dirname(__FILE__)),
                array('jquery'),
                $version
            );
        }
    ...
    public function metabox()
    {
        ...
        if (! get_the_ID()) {
            ...
            echo '<div id="yarpp-related-posts"><img src="' . esc_url(
...
//		if ('category' === $taxonomy) $exclude .= ','.get_option('default_category');
    ...
    public function get_term_ids_from_tt_ids($taxonomy, $tt_ids)
    {
        ...
        return $wpdb->get_col(
            "select term_id from $wpdb->term_taxonomy where taxonomy = '{$taxonomy}' and term_taxonomy_id in (" . join(
                ',',
                $tt_ids
            ) . ")"
        );
